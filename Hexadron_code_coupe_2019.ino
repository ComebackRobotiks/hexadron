...
void setup() {
  ...
  Use_US_AV = 0; //on ignore tjrs les US avant
...
void loop() {
  ...
  //*********************************mouvements***************************************
  Use_US = 0;//on ignore les US pour sortir de la zone de départ
  deplacement(1, 0, (int)(0.15 * tic_metre_AV)); //activ XP
  Use_US = 1; //on active les US (arrière, avant ignoré dans setup)
  temps = millis();
  while ((millis() - temps) < 2000)
  {
  }
  deplacement(side, 90, (int)(1.34 * tic_metre_G)); //vers accel
  //sortie bras
  ...
  temps = millis();
  while ((millis()-temps)< 2000){//avance sur tempo (calage distance avant actionner accelerateur)
    digitalWrite(mot_sens_A, LOW);
    digitalWrite(mot_sens_D, LOW);
    digitalWrite(mot_sens_G, LOW);
    analogWrite(mot_pwm_A, 0);
    analogWrite(mot_pwm_D, 54);
    analogWrite(mot_pwm_G, 62);
    
  }
  
  deplacement(-1, 0, (int)(0.10 * tic_metre_AV)); //recul de calage
  servo.write(110);//sortie du bras
  temps = millis();
  while ((millis() - temps) < 3000)
  {
  }
  deplacement(1, 0, (int)(0.1 * tic_metre_AV));//mise en position du bras, pret à pousser  !!!! REGLAGE SUR TABLE !!!!
  temps = millis();
  while ((millis() - temps) < 3000)
  {
  }
  deplacement(side, 90, (int)(0.2* tic_metre_G)); //on pousse
  temps = millis();
  while ((millis() - temps) < 3000)
  {
  }
  
  deplacement(-1,0, (int)(0.06*tic_metre_AV));//dégagement accel - recul
  servo.write(30);//rentrée bras
  deplacement(side,90, (int)(0.20*tic_metre_G));//degagement par la gauche
